 ...
 * Hiçbir metadata (maç adı, kodu, tarihi, marka, üretim bilgisi) içermez.
 */
...
export function createCategoryExportElement(match, categoryName, markets) {
  // markets parametresi iki farklı formatta gelebilir:
  // 1. Object: { "Maç Sonucu": { "1": 2.10, "X": 3.30 } }
  // 2. Array: [{ category: "Maç Sonucu", odds: [{ name: "1", value: 2.10 }] }]
  let marketEntries;
  ...
  if (Array.isArray(markets)) {
    ...
    for (const item of markets) {
      if (!item || !item.category || !Array.isArray(item.odds)) continue;
      const oddsObj = {};
      for (const odd of item.odds) {
        if (odd && odd.name && isValidOdd(odd.value)) {
          oddsObj[odd.name] = odd.value;
        }
      }
      if (Object.keys(oddsObj).length > 0) {
        converted[item.category] = oddsObj;
      }
    }
    marketEntries = Object.entries(converted);
  } else {
    marketEntries = Object.entries(markets || {}).filter(([_, odds]) => {
  ...
  `;
  // categoryName string ise doğrudan kullan, obje ise match parametresi categoryName olarak gelmiş olabilir
  categoryTitle.textContent = typeof categoryName === 'string' ? categoryName : 'ORANLAR';
